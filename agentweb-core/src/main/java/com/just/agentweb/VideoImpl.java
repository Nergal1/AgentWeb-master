...
public class VideoImpl implements IVideo, EventInterceptor {
    ...
    public void onShowCustomView(View view, WebChromeClient.CustomViewCallback callback) {
        Activity mActivity;
        if ((mActivity = this.mActivity.get()) == null || mActivity.isFinishing()) {
            return;
        }
    ...
    public void onHideCustomView() {
        ...
        if (mActivity.get() != null && mActivity.get().getRequestedOrientation() != ActivityInfo.SCREEN_ORIENTATION_PORTRAIT) {
            mActivity.get().setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
        }
        if (!mFlags.isEmpty()) {
            for (Pair<Integer, Integer> mPair : mFlags) {
                if (mActivity.get() != null) {
                    mActivity.get().getWindow().setFlags(mPair.second, mPair.first);
                }
            }
            mFlags.clear();
        }
