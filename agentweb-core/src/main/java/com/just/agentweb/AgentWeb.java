...
public final class AgentWeb {
	...
	public AgentWeb clearWebCache() {
		if (this.getWebCreator().getWebView() != null) {
			AgentWebUtils.clearWebViewAllCache(mActivity.get(), this.getWebCreator().getWebView());
		} else {
			AgentWebUtils.clearWebViewAllCache(mActivity.get());
		}
		return this;
	}
	...
	private void doCompat() {
		mJavaObjects.put("agentWeb", mAgentWebJsInterfaceCompat = new AgentWebJsInterfaceCompat(this, mActivity.get()));
	}
	...
	private WebCreator configWebCreator(BaseIndicatorView progressView, int index, ViewGroup.LayoutParams lp, int indicatorColor, int height_dp, WebView webView, IWebLayout webLayout) {
		if (progressView != null && mEnableIndicator) {
			return new DefaultWebCreator(mActivity.get(), mViewGroup, lp, index, progressView, webView, webLayout);
		} else {
			return mEnableIndicator ?
					new DefaultWebCreator(mActivity.get(), mViewGroup, lp, index, indicatorColor, height_dp, webView, webLayout)
					: new DefaultWebCreator(mActivity.get(), mViewGroup, lp, index, webView, webLayout);
		}
	}
	...
	private IVideo getIVideo() {
		return mIVideo == null ? new VideoImpl(mActivity.get(), mWebCreator.getWebView()) : mIVideo;
	}
	...
	private AgentWeb ready() {
		AgentWebConfig.initCookiesManager(mActivity.get().getApplicationContext());
	...
	private WebChromeClient getChromeClient() {
		...
		DefaultChromeClient mDefaultChromeClient =
				new DefaultChromeClient(this.mActivity.get(),
